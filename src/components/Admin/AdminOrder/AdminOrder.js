...
function AdminOrder() {
    ...
    useEffect(() => {
        fetch(`https://ecommerceas.herokuapp.com/api/order/get/get`)
       .then((res) => res.json())
       .then((asd) => setApi(asd));
    },[]);
  ...
  <tbody>
    {
        api.map((apps) => (
          ...
          <td><button id="remicon" onClick={() => window.location.href=`/admin/order/${apps._id}` }><RemoveRedEyeIcon id="rem" /></button></td>
        </tr>
        ))
    }
  </tbody>
